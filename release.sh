...
PODSPEC_PATH="$SCRIPT_DIR/DaroMCore.podspec"
...
if [ ! -f "$SCRIPT_DIR/build/DaroMCore.xcframework.zip" ]; then
  echo "$SCRIPT_DIR/build/DaroMCore.xcframework.zip file not found"
  exit 1
fi

# # Create a tag and release using GitHub CLI with the file
gh release create $VERSION "$SCRIPT_DIR/build/DaroMCore.xcframework.zip" --title "Release $VERSION" --notes "Release version $VERSION"
